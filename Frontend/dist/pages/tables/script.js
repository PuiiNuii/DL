...
//       </div>
//   </td>
//   <td>${totalDebtPerSale.toLocaleString()} KHR</td> <!-- Changed from sale.oldDebt to totalDebtPerSale -->
...
// Function to get the previous day's total debt for a customer
async function getPreviousDayDebt(customerName, currentDate) {
  try {
    const previousDay = new Date(currentDate);
    previousDay.setDate(previousDay.getDate() - 1);
    
    const response = await fetch(`${API_URL}?customerName=${encodeURIComponent(customerName)}`);
    if (!response.ok) throw new Error('Failed to fetch previous sales');
    
    const customerSales = await response.json();
    const previousDaySales = customerSales.filter(sale => {
      const saleDate = new Date(sale.createdAt);
      return saleDate.toDateString() === previousDay.toDateString();
    });
    
    if (previousDaySales.length > 0) {
      const latestSale = previousDaySales.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
      return latestSale.oldDebt + latestSale.newDebt - latestSale.payment;
    }
    return 0; // If no previous day sale exists, return 0
  } catch (error) {
    console.error('Error fetching previous day debt:', error);
    return 0;
  }
}
...
document.getElementById('iceSaleForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    // Get previous day's total debt as oldDebt
    formData.oldDebt = await getPreviousDayDebt(customerName, currentDate);

    if (saleId) {
      const existingSale = salesData.find(s => s._id === saleId);
      if (!existingSale) throw new Error('Sale not found in local data');
      // Keep the original oldDebt when editing
      formData.oldDebt = existingSale.oldDebt;
...
async function editSale(id) {
  try {
    ...
    function updateDebtDisplay() {
      ...
      const payment = parseFloat(paymentInput.value) || 0;
      const totalDebt = oldDebt + newDebt - payment;
      totalDebtDisplay.textContent = `សរុបប្រាក់ជំពាក់: ${totalDebt.toLocaleString()} KHR`;
    }
...
function saveReport() {
  ...
  reportContainer.style.position = 'absolute';
  reportContainer.style.left = '-9999px';
  reportContainer.style.width = '1500px';
  reportContainer.style.backgroundColor = 'white';
  reportContainer.style.padding = '20px';
  reportContainer.style.boxSizing = 'border-box';
  reportContainer.style.fontSize = '12px';
